...
export default function QuickViewModal({ product, children }: QuickViewModalProps) {
  ...
  // 1. Estrellas Dinámicas (Sentimiento del mercado 1-10 mapeado a 1-5)
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2">
          ...
          <div className="relative aspect-square bg-muted/20 flex items-center justify-center p-12 overflow-hidden">
            ...
            {isPremium && (
               <div className="absolute top-6 left-6 z-10 bg-green-50/90 backdrop-blur-md text-green-700 border border-green-200 px-3 py-1.5 rounded-xl flex items-center gap-1.5 shadow-sm">
                  <ShieldCheck className="w-3.5 h-3.5" />
                  <span className="text-[9px] font-black uppercase tracking-widest">Nutrición Premium</span>
               </div>
            )}
          </div>
          ...
          <div className="p-8 md:p-12 flex flex-col justify-center space-y-6 bg-white relative">
            <div className="space-y-3">
              <div className="flex items-center gap-2 flex-wrap justify-between">
                ...
                {/* ESTRELLAS DINÁMICAS (Social Proof Técnico) */}
                <div className="flex items-center gap-1 bg-muted/50 px-2.5 py-1 rounded-full border border-black/[0.03]">
                  <div className="flex items-center text-amber-400">
                    <Star className="w-3 h-3 fill-current" />
                  </div>
                  <span className="text-[10px] font-black text-foreground">{dynamicRating}</span>
                </div>
              </div>
              ...
              <div className="flex flex-col pt-2">
                <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1 opacity-60">
                  {priceLabel}
                </span>
                <div className="flex items-baseline gap-3">
                  ...
                  {ppkg && (
                    <span className="text-xs font-bold text-muted-foreground/60 bg-muted px-2 py-0.5 rounded-lg">
            ...
            <div className="space-y-4 pt-2 mt-auto">
              <div className={cn(
                ...
                <div className="flex items-center gap-1 bg-white rounded-xl p-1 shadow-sm">
                  <Button variant="ghost" size="icon" onClick={decrement} disabled={quantity <= 1 || isOutOfStock} className="h-9 w-9 rounded-lg">
                    <Minus className="w-4 h-4" />
                  </Button>
                  <span className="font-black text-lg w-10 text-center">{quantity}</span>
                  <Button variant="ghost" size="icon" onClick={increment} disabled={quantity >= maxQuantity || isOutOfStock} className="h-9 w-9 rounded-lg">
