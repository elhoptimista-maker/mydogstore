...
/**
 * @fileOverview Carrito lateral con gamificación de envío gratis para la RM.
 */
      ...
      >
          ...
          <ScrollArea className="h-full w-full">
            <div className="px-6 py-2">
              {cart.length === 0 ? (
                ...
                cart.map((item) => (
                  <div key={item.id} className="flex gap-3 items-center py-4 border-b border-black/[0.03] last:border-0">
                    <button onClick={() => removeFromCart(item.id)} className="text-muted-foreground/30 hover:text-destructive transition-colors p-1 shrink-0">
                    ...
                    <div className="flex-1 min-w-0 space-y-2">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1 bg-white rounded-lg p-0.5 border border-border/60 shadow-sm">
                          <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-6 h-6 flex items-center justify-center rounded-md text-muted-foreground hover:bg-muted disabled:opacity-20" disabled={item.quantity <= 1}>
                            <Minus className="w-2.5 h-2.5" />
                          </button>
                          <span className="text-[10px] font-bold w-5 text-center">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="w-6 h-6 flex items-center justify-center rounded-md text-muted-foreground hover:bg-muted">
        ...
        {cart.length > 0 && (
          <div className="px-6 py-6 bg-white shrink-0 shadow-[0_-15px_40px_rgba(0,0,0,0.08)] relative z-10">
              ...
              {cartType === 'retail' && (
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="outline" className="text-[8px] font-black bg-primary/5 text-primary border-primary/10 uppercase px-1.5 h-4">
            ...
            <SheetTrigger asChild>
              <Button onClick={() => router.push('/checkout')} className="w-full h-14 rounded-2xl bg-primary text-white font-black text-base shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2">
