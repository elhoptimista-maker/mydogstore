...
/**
 * @fileOverview Acciones de navegación unificadas.
 * Eliminado el amarillo para consistencia de marca.
 */
  ...
  return (
    <div className="flex items-center gap-2 sm:gap-4 md:gap-6 shrink-0">
      <Link href="/wishlist" title="Tus Favoritos" className="relative w-10 h-10 sm:w-12 sm:h-12 bg-white/10 text-white rounded-full flex items-center justify-center hover:bg-white/20 transition-all group">
        <Heart className={cn("w-4 h-4 sm:w-5 sm:h-5", wishlist.length > 0 && "fill-current")} />
        {wishlist.length > 0 && (
          <span className="absolute -top-1 -right-1 bg-white text-primary font-black text-[8px] sm:text-[10px] w-4 border-2 h-4 sm:w-5 sm:h-5 flex items-center justify-center border-primary rounded-full animate-in zoom-in">
            {wishlist.length}
          </span>
        )}
      </Link>
      ...
      <CartDrawer>
        <button title="Ir a la Caja" className="relative w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center shadow-lg hover:scale-105 transition-all bg-white text-primary">
          <ShoppingCart className="w-4 h-4 sm:w-5 sm:h-5" />
          {cartCount > 0 && (
            <span className="absolute -top-1 -right-1 bg-primary text-white font-black text-[8px] sm:text-[10px] w-4 border-2 h-4 sm:w-5 sm:h-5 flex items-center justify-center border-white rounded-full animate-in zoom-in">
              {cartCount}
            </span>
          )}
        </button>
