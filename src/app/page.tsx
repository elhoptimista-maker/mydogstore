    ...
    <div className="bg-[#F6F6F6] min-h-screen">
      ...
      <section className="py-20 max-w-7xl mx-auto px-4 md:px-8 space-y-12">
        <div className="text-center space-y-2">
          <span className="text-xs font-bold text-muted-foreground uppercase tracking-[0.2em]">Nuestros Amigos</span>
        ...
        <div className="flex overflow-x-auto md:overflow-visible md:flex-wrap md:justify-center gap-8 md:gap-14 no-scrollbar pb-4 snap-x">
          {[
            { name: 'Perros', emoji: '🐶', color: 'bg-muted' },
            { name: 'Gatos', emoji: '🐱', color: 'bg-muted' },
            { name: 'Aves', emoji: '🦜', color: 'bg-muted' },
            { name: 'Conejos', emoji: '🐰', color: 'bg-muted' },
            { name: 'Roedores', emoji: '🐹', color: 'bg-muted' },
            { name: 'Peces', emoji: '🐠', color: 'bg-muted' },
          ].map((cat, i) => (
      ...
      <section className="py-10 max-w-7xl mx-auto px-4 md:px-8 grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white rounded-3xl relative overflow-hidden p-10 flex items-center shadow-sm hover:shadow-md transition-all">
          <div className="space-y-6 flex-1 z-10">
            ...
            <h3 className="text-4xl font-black tracking-tighter leading-none text-foreground">Camas <span className="text-primary">Ortopédicas</span></h3>
        ...
        <div className="bg-accent rounded-3xl relative overflow-hidden p-10 flex items-center shadow-sm hover:shadow-md transition-all">
          <div className="space-y-6 flex-1 z-10">
            ...
            <h3 className="text-4xl font-black tracking-tighter leading-none text-foreground">Alimento <span className="text-primary">Natural</span></h3>
            <Button className="rounded-full bg-white text-primary font-black px-8 hover:bg-primary hover:text-white transition-colors">Explorar</Button>
          </div>
          <div className="relative w-48 h-48">
            <Image src="https://picsum.photos/seed/food/400/400" alt="Alimento" fill className="object-contain" />
          </div>
        </div>
      </section>
      ...
      <section className="py-20 max-w-7xl mx-auto px-4 md:px-8 space-y-12">
        <div className="flex flex-row justify-between items-end">
          <div className="space-y-1">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Favoritos de la semana</span>
            <h2 className="text-4xl font-black tracking-tight text-foreground">Lo más <span className="text-primary">Popular</span></h2>
          </div>
          <Button variant="outline" className="rounded-full font-black border-2 px-8">Ver Todos</Button>
        </div>
      ...
      <section className="bg-[#FDF8F3] py-20 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 md:px-8 grid grid-cols-1 lg:grid-cols-3 gap-12 items-center text-center lg:text-left">
          <div className="hidden lg:block relative h-80">
            <Image src="https://picsum.photos/seed/cat-deal/600/600" alt="Cat" fill className="object-contain" />
          </div>
          ...
          <div className="hidden lg:block relative h-80">
            <Image src="https://picsum.photos/seed/dog-deal/600/600" alt="Dog" fill className="object-contain" />
          </div>
        </div>
      </section>
      ...
      <section className="w-full bg-primary py-20 text-white overflow-hidden relative">
        <div className="max-w-7xl mx-auto px-4 md:px-8 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          <div className="relative h-[400px] hidden md:block">
            <Image src="https://picsum.photos/seed/club/800/600" alt="Club" fill className="object-contain drop-shadow-2xl" />
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 md:px-8 grid grid-cols-1 md:grid-cols-2 gap-16 items-start">
          <div className="space-y-6">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Centro de Ayuda</span>
          ...
          <Accordion type="single" collapsible className="w-full space-y-4">
            {[
              ...
              <AccordionItem key={i} value={`item-${i}`} className="border-none">
                <AccordionTrigger className="flex gap-4 p-4 bg-muted/20 rounded-xl hover:no-underline data-[state=open]:bg-primary data-[state=open]:text-white transition-all">
                  <span className="text-sm font-bold text-left">{item.q}</span>
                </AccordionTrigger>
                <AccordionContent className="p-4 text-muted-foreground font-medium bg-white rounded-b-xl border border-t-0 border-black/5">
                  {item.a}
                </AccordionContent>
              </AccordionItem>
            ))}
      ...
      <section className="py-16 max-w-7xl mx-auto px-4 md:px-8 border-t border-black/5">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          {[
            { i: <Truck />, t: 'Envío Express', s: 'Despacho mismo día antes de las 14:00' },
            { i: <ShieldCheck />, t: 'Pago Seguro', s: 'Transacciones 100% protegidas por Webpay' },
            { i: <Headphones />, t: 'Soporte 24/7', s: 'Asistencia garantizada de lunes a domingo' },
            { i: <CheckCircle2 />, t: 'Calidad Garantizada', s: 'Solo marcas certificadas y productos frescos' }
          ].map((item, i) => (
            <div key={i} className="flex items-center gap-4 group">
              ...
              <div className="flex flex-col">
                ...
                <p className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">{item.s}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4 md:px-8 space-y-8">
          ...
          <Carousel className="w-full">
            ...
            <div className="flex justify-center gap-4 mt-8">
              <CarouselPrevious className="relative translate-y-0 left-0 bg-black text-white hover:bg-primary border-none" />
              <CarouselNext className="relative translate-y-0 right-0 bg-black text-white hover:bg-primary border-none" />
            </div>
          </Carousel>
        </div>
      </section>
      ...
      <section className="py-20 max-w-7xl mx-auto px-4 md:px-8 space-y-12">
        <div className="text-center space-y-2">
          <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Aprende con Nosotros</span>
          ...
          {[
            ...
            <div key={i} className="flex flex-col gap-4 group cursor-pointer">
              <div className="aspect-[4/3] rounded-2xl overflow-hidden relative">
                <Image src={`https://picsum.photos/seed/blog-${i}/600/450`} alt={post.t} fill className="object-cover group-hover:scale-110 transition-transform duration-700" />
      ...
      <section className="w-full flex overflow-hidden">
        {[1,2,3,4,5,6].map(i => (
          <div key={i} className="flex-1 aspect-square relative group cursor-pointer overflow-hidden">
            <Image src={`https://picsum.photos/seed/insta-${i}/400/400`} alt="ig" fill className="object-cover group-hover:scale-110 transition-transform duration-700" />
      ...
      <footer className="w-full">
        ...
        <div className="bg-[#FEF9F3] py-20">
          <div className="max-w-3xl mx-auto px-4 text-center space-y-10">
            ...
            <div className="relative max-w-md mx-auto">
              <div className="relative flex items-center bg-white rounded-full h-16 px-2 shadow-sm border border-black/5">
                <input type="email" placeholder="tu@email.com" className="flex-1 h-full bg-transparent outline-none px-6 font-bold text-sm text-foreground" />
                <Button className="rounded-full bg-primary text-white font-black px-8 h-12 shadow-md hover:scale-105 active:scale-95 transition-all">
                  Suscribirse
                </Button>
              </div>
            </div>
        ...
        <div className="px-4 md:px-8 pb-10">
          <div className="bg-primary text-white rounded-3xl p-12 md:p-20 max-w-7xl mx-auto shadow-2xl">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-16">
              <div className="lg:col-span-2 space-y-8">
                <Link href="/" className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col -space-y-1">
                    <span className="font-black text-2xl tracking-tighter">MyDog</span>
                    <span className="text-[10px] font-bold text-white/60 uppercase tracking-widest">Distribuidora</span>
                  </div>
                </Link>
              ...
              <div>
                <h4 className="font-black text-xs uppercase tracking-widest mb-8 text-accent">Navegación</h4>
                <ul className="space-y-4 text-sm font-bold text-white/70">
                  {['Catálogo', 'Venta Mayorista', 'Marcas', 'Blog'].map(l => (
                    <li key={l}><Link href="#" className="hover:text-white hover:underline transition-all">{l}</Link></li>
                  ))}
                </ul>
              </div>
              <div>
                <h4 className="font-black text-xs uppercase tracking-widest mb-8 text-accent">Información</h4>
                <ul className="space-y-4 text-sm font-bold text-white/70">
                  {['Quiénes somos', 'Términos', 'Políticas', 'FAQ'].map(l => (
                    <li key={l}><Link href="#" className="hover:text-white hover:underline transition-all">{l}</Link></li>
                  ))}
                </ul>
              </div>
              <div>
                <h4 className="font-black text-xs uppercase tracking-widest mb-8 text-accent">Contacto</h4>
